...
void main(int argc, char** argv)
{
        ...
        #define G 6.674e-11
        #define c 3e+8
        int xRes = 200, xRes_in;  // Default resolution
        int yRes = 200, yRes_in;
        double hole_mass = 8.57e+36, hole_mass_in;
        int disk_coef = 5, disk_coef_in;
        vec3 camera_pos(0, 0, 5 * 12.7e+9), camera_pos_in;
        vec3 camera_forward(0, 0, -1), camera_forward_in;
        vec3 camera_up(1, 0, 0), camera_up_in;
        ...
        if(argc == 2) // There is input file in parameters
        {
                ...
                if(file) {
                        int xResFromFile = 0;
                        int yResFromFile = 0;
                        if(fscanf(file, "%lf %d %d %d %f %f %f %f %f %f %f %f %f %f %f %f %lf ", &hole_mass_in, &disk_coef_in, &xRes_in, &yRes_in,
